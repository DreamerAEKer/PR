...
# 1. Update App.css
css_content = """
/* Meter History & Summary Styles in Data Entry */
.meter-summary-card {
...
.meter-history-table th:first-child {
  text-align: left;
}
...
.meter-history-table td:first-child {
  text-align: left;
  font-family: inherit;
}

.meter-history-table tr.selected-row {
  background: rgba(37, 99, 235, 0.15);
  font-weight: 700;
}

.btn-autofill-reading {
  display: inline-flex;
  align-items: center;
  gap: 4px;
...
if '.meter-summary-card' not in css:
      ...
      <div className="grid-2col">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          ...
          <div className="glass-card">
            ...
            {dailyRecords.length === 0 ? (
              <p className="text-muted" style={{ fontSize: '0.9rem' }}>ยังไม่มีการบันทึกข้อมูลบริการสำหรับบริษัทนี้ในวันนี้</p>
            ) : (
                  ...
                  <table className="meter-history-table">
                    ...
                    <tbody>
                      {companyMeterHistory.map((item) => {
                        ...
                        return (
                          <tr key={item.date} className={isSelected ? 'selected-row' : ''}>
                            <td>
...
print("Patch meter history applied successfully.")
