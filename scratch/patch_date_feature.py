...
# Add moveSingleRecord and moveDailyRecords
old_move_defs = """  const moveSingleRecordToCompany = (record, targetCompanyId) => {
    if (!record || !targetCompanyId) return;
    setRecords(prev => prev.map(r => {
      ...
      if (isMatch) {
        return { ...r, companyId: targetCompanyId };
      }
      return r;
    }));
  };
  ...
  const moveDailyRecordsToCompany = (date, fromCompanyId, targetCompanyId) => {
    if (!date || !fromCompanyId || !targetCompanyId || fromCompanyId === targetCompanyId) return;
    setRecords(prev => prev.map(r => {
      if (r.date === date && r.companyId === fromCompanyId) {
        return { ...r, companyId: targetCompanyId };
      }
      return r;
    }));
    setMachineReadings(prev => prev.map(mr => {
      if (mr.date === date && mr.companyId === fromCompanyId) {
        return { ...mr, companyId: targetCompanyId };
      }
      return mr;
    }));
  };"""

new_move_defs = """  const moveSingleRecord = (record, { targetCompanyId, targetDate }) => {
    if (!record) return;
    setRecords(prev => prev.map(r => {
      const isMatch = (record.timestamp && r.timestamp && String(r.timestamp) === String(record.timestamp)) ||
        (!record.timestamp && r.date === record.date && r.companyId === record.companyId && r.serviceId === record.serviceId);
...
if 'moveSingleRecord =' not in app_context:
    app_context = app_context.replace(old_move_defs, new_move_defs)
    app_context = app_context.replace(
        "records, setRecords, addRecord, deleteRecords, deleteSingleRecord, moveSingleRecordToCompany, moveDailyRecordsToCompany,",
    ...
    with open('src/context/AppContext.jsx', 'w', encoding='utf-8') as f:
        f.write(app_context)
    print("AppContext.jsx updated.")

# 2. Update DataEntry in App.jsx
...
new_data_entry = """const DataEntry = () => {
  ...
  const handleOpenReassignModal = (target) => {
    setReassignModal({
      isOpen: true,
      target, // 'ALL' or single record object
...
print("Patch date feature applied successfully.")
